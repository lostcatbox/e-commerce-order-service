    ...
    override fun processPayment(command: ProcessPaymentCommand): Payment {
        ...
        try {
            ...
            if (order.usedCouponId != null) {
                val usedCoupon = coreApiClient.getUserCoupon(order.usedCouponId)
                val couponInfo = coreApiClient.getCouponInfo(usedCoupon.couponId)
            ...
            // 3. 쿠폰 사용 처리
            if (order.usedCouponId != null) {
                coreApiClient.useCoupon(order.usedCouponId)
            }
