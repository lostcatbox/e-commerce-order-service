...
class PaymentServiceIntegrationTest : IntegrationTestSupport() {
    ...
    fun `쿠폰이 설정된 주문으로 결제 처리`() {
        // given - Mock을 사용하여 쿠폰 서비스 호출 여부만 확인
        ...
        // 쿠폰이 설정된 주문 생성
        val orderWithCoupon = Order(userId = testUserId, usedCouponId = userCouponId)
        ...
        val expectedFinalAmount = originalAmount - discountAmount
        ...
        val savedPayment = paymentRepository.findByPaymentId(result.paymentId)
        assertNotNull(savedPayment)
        assertEquals(discountAmount, savedPayment!!.discountAmount)
