...
class CouponControllerTest {
    ...
    inner class CouponInfoTest {
        ...
        @Test
        @DisplayName("유효하지 않은 쿠폰 ID로 조회 시 400 에러가 발생한다")
        fun `유효하지_않은_쿠폰_ID로_조회_시_400_에러가_발생한다`() {
            // given
            val invalidCouponId = 0L

            given(couponService.getCouponInfo(invalidCouponId))
                .willThrow(IllegalArgumentException("쿠폰 ID는 0보다 커야 합니다. 입력된 ID: $invalidCouponId"))

            // when & then
            mockMvc.perform(get("/api/coupons/{couponId}", invalidCouponId))
                .andExpect(status().isBadRequest)

            verify(couponService).getCouponInfo(invalidCouponId)
        }
    }
    ...
    inner class CouponIssueTest {
        ...
        fun `정상적으로_쿠폰을_발급할_수_있다`() {
            ...
            given(couponFacade.issueCoupon(userId, couponId)).willReturn(userCoupon)
        ...
        fun `이미_발급받은_쿠폰_발급_시_400_에러가_발생한다`() {
            ...
            given(couponFacade.issueCoupon(userId, couponId))
        ...
        @Test
        @DisplayName("발급 불가능한 쿠폰 발급 시 400 에러가 발생한다")
        fun `발급_불가능한_쿠폰_발급_시_400_에러가_발생한다`() {
            // given
            val userId = 1L
            val couponId = 1L
            val request = CouponIssueRequest(userId = userId, couponId = couponId)
            ...
            given(couponFacade.issueCoupon(userId, couponId))
                .willThrow(IllegalStateException("발급 불가능한 쿠폰입니다. 쿠폰 ID: $couponId"))
        ...
        @Test
        @DisplayName("유효하지 않은 사용자 ID로 발급 시 400 에러가 발생한다")
        fun `유효하지_않은_사용자_ID로_발급_시_400_에러가_발생한다`() {
            // given
            val invalidUserId = 0L
            val couponId = 1L
            val request = CouponIssueRequest(userId = invalidUserId, couponId = couponId)

            given(couponFacade.issueCoupon(invalidUserId, couponId))
            ...
            verify(couponFacade).issueCoupon(invalidUserId, couponId)
        }
    }
    ...
    inner class UserCouponTest {
        ...
        fun `사용자의_쿠폰_목록을_조회할_수_있다`() {
            ...
            given(userCouponService.getUserCoupons(userId)).willReturn(userCoupons)
        ...
        fun `쿠폰이_없는_사용자의_경우_빈_목록을_반환한다`() {
            ...
            given(userCouponService.getUserCoupons(userId)).willReturn(emptyList())
        ...
        fun `유효하지_않은_사용자_ID로_조회_시_400_에러가_발생한다`() {
            ...
            given(userCouponService.getUserCoupons(invalidUserId))
                .willThrow(IllegalArgumentException("사용자 ID는 0보다 커야 합니다. 입력된 ID: $invalidUserId"))
            ...
            verify(userCouponService).getUserCoupons(invalidUserId)
        }
    }
}
