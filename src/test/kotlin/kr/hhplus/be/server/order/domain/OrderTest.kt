...
import kr.hhplus.be.server.order.domain.OrderItem.Companion.MAX_QUANTITY
import kr.hhplus.be.server.order.domain.OrderItem.Companion.MIN_QUANTITY
...
class OrderTest {
    ...
    fun `경계값 테스트 - 최대 수량 주문`() {
        ...
        val orderItems =
            listOf(
                OrderItem(productId = 1L, quantity = OrderItem.MAX_QUANTITY, unitPrice = 1000L),
            )
        ...
        // then
        assertEquals(OrderItem.MAX_QUANTITY * 1000L, order.calculateTotalAmount())
    }
