...
class ProductService(
    ...
    override fun saleOrderProducts(command: SaleProductsCommand) {
        ...
        command.orderItems.forEach { orderItemCommand ->
            ...
            require(orderItemCommand.quantity > 0) { "주문 수량은 0보다 커야 합니다. 상품 ID: ${orderItemCommand.productId}, 수량: ${orderItemCommand.quantity}" }
            ...
            if (!product.hasEnoughStock(orderItemCommand.quantity)) {
                throw IllegalStateException("상품 재고가 부족합니다. 상품 ID: ${orderItemCommand.productId}, 요청 수량: ${orderItemCommand.quantity}, 현재 재고: ${product.getStock()}")
            }
